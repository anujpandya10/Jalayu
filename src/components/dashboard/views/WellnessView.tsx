...
export default function WellnessView({
  ...
  return (
    <div style={{ padding: '16px 14px' }}>
      <h2 style={{ fontSize: 18, fontWeight: 600, color: '#111827', margin: '0 0 4px' }}>Wellness</h2>
      ...
      <div className="card" style={{ padding: 14 }}>
        <p style={{ fontSize: 12, fontWeight: 500, color: '#374151', margin: '0 0 10px' }}>Recent mood entries</p>
        {sorted.length === 0 ? (
          <p style={{ fontSize: 13, color: '#6b7280', margin: 0 }}>Log a mood above — over time you&apos;ll see rhythm, not noise.</p>
        ) : (
            ...
            {sorted.slice(0, 14).map((m) => (
              <li
                ...
                style={{
                  ...
                  borderBottom: '0.5px solid #F3F4F6', fontSize: 13, color: '#374151',
                }}
              >
                <span>{new Date(m.created_at).toLocaleString('en-US', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' })}</span>
                <span style={{ fontWeight: 600, color: '#534AB7' }}>{m.score}/5</span>
              </li>
            ))}
          </ul>
        )}

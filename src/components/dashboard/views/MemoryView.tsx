...
export default function MemoryView({
  ...
  return (
    <div style={{ padding: '16px 14px' }}>
      <h2 style={{ fontSize: 18, fontWeight: 600, color: '#111827', margin: '0 0 4px' }}>Memory</h2>
      <p style={{ fontSize: 12, color: '#9CA3AF', margin: '0 0 14px' }}>Search everything you&apos;ve saved and reflected on.</p>
      ...
      <div style={{ position: 'relative', marginBottom: 12 }}>
        ...
        <input
          ...
          style={{
            ...
            border: '0.5px solid #E5E3FF', fontSize: 13, outline: 'none', color: '#374151',
          }}
        />
      </div>
      ...
      {timeline.length === 0 ? (
        <p style={{ fontSize: 13, color: '#6b7280', textAlign: 'center', padding: '32px 8px' }}>
          ...
          {timeline.map((item) => (
            <div key={`${item.kind}-${item.id}`} className="card">
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                <span style={{ fontSize: 10, fontWeight: 600, color: '#534AB7', textTransform: 'uppercase' }}>{item.title}</span>
